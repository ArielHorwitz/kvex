...
In order for this patch to work, the object should be initialized once as early as
possible, and stored in a namespace that won't get garbage collected.

## Considerations
It seems that either Kivy or the system ensures that the Window gaining focus always
happens first, and that the Window losing focus always happens last. And so the solution
has to consider that a key press (e.g. "tab") may be the key press that will remove
focus from our Window (e.g. alt + tab) later during this frame.

## Solution
Hence we consume the Window's `on_key_` events and only dispatch them a frame later if
